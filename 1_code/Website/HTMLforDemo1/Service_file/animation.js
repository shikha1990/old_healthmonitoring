...
var ImageRotator = function(el,images,transProps){
    if(el.id!=="carouselImg"){
        el.style.zIndex=1;
    }
    var elemChild = el.children;
	this.frontCont=elemChild[0];
    this.frontCont.style.backgroundImage="url('../zimages/progressbarsmall.gif')";//NO I18N
    ...
    if(window.location.href.indexOf("mobile") !== -1){
        ...
        if(document.addEventListener){
            document.body.addEventListener('orientationchange', function(){return called.changeOrientation1()},false);
        }
    ...
    if(transProps.thumbnail && transProps.thumbnail == "on" && (!this.type || (this.type && this.type == "carousel"))){
        ...
        if(transProps.thumbPos == "top" || transProps.thumbPos == "bottom"){
            ...
            thumbCont.style.cssText = "z-index: 300;width: 65px;height: "+this.maxH+"px;background: transparent;cursor:pointer;float:left;overflow:hidden";//NO I18N
        }
...
ImageRotator.prototype={
	...
	start:function(trans){
        ...
        if(this.overlay && this.slideURL[this.curPos].overlay && this.slideURL[this.curPos].overlay.length && window.location.href.indexOf("mobile") == -1){
            if(!overlayElem){
            ...
            if(this.slideURL[this.curPos].link && this.slideURL[this.curPos].link.url){
                ...
                overlayElem.setAttribute("onclick","window.open('"+this.slideURL[this.curPos].link.url+"','"+this.slideURL[this.curPos].link.target+"')");
            ...
            for(var i = 0,elem;elem = elems[i];i++){
                if(elem.type == "text"){
                    overlay+="<div style='position:absolute;top:"+elem.top+"px;left:"+elem.left+"px;background:"+elem.background+";color:"+elem.color+";width:"+elem.width+"px;height:"+elem.height+"px;padding:10px;font-size:16px;z-index:1' class='zpelement-wrapper bannerText'>"+elem.content+"</div>";
                }else{
                    ...
                    overlay+=" blink='"+elem.buttonLink+"' btarget=\""+elem.linkTarget+"\" onclick = \"window.open('"+elem.buttonLink+"','"+elem.linkTarget+"')\">"+elem.name+"</button></div></div>";//NO I18N
                }
            }
	...
	fnSetOriginal:function(trans){
      ...
      if(ver!==-1 && ver<=10){
        this.frontCont.style.cssText="position:absolute;top:0px;left:0px;opacity:1;filter:alpha(opacity=100);display:block;width:"+this.maxW+"px;height:"+this.maxH+"px;overflow:hidden;-webkit-backface-visibility:hidden";//No I18N
            ...
            this.frontCont.style.cssText="position:absolute;top:0px;left:0px;opacity:1;filter:alpha(opacity=100);display:block;width:"+this.maxW+"px;height:"+this.maxH+"px;overflow:hidden;background:#ffffff;-webkit-backface-visibility:hidden";//No I18N
        }
		this.backCont.style.display="none";
		this.backCont.style.opacity=1;
       if(!trans)return;
